...
Public Class ClsShakedPendulum
    ...
    'Spring constant of the horizontal shaking pendulum
    ...
    'Masses m, M
    Const SpringM As Decimal = 2
    Const ThreadM As Decimal = 1
    ...
    Public Sub New()
        ...
        'x
        ValueParameter(0) = New ClsValueParameter(2, "x", New ClsInterval(-MaxX, MaxX))
        ...
        MyAdditionalParameterValue = 15  'that means MyD in the middle
        MyAdditionalParameter = New ClsValueParameter(MyAdditionalParameterValue,
                                                      Main.LM.GetString("SpringConstant"), New ClsInterval(5, 25))
        MyD = 5
    ...
    Private Sub SetStartenergyRange()
        ...
        With Constants
            Emin = -ThreadM * g * .Component(0)
            Emax = MyD / 2 + ThreadM * g * .Component(0)
        End With
    ...
    Sub SetAndDrawStartparameter2(Mouseposition As Point) Implements IPendulum.SetAndDrawStartparameter2
        ...
        With ActualPosition
            ...
            'L should be maximal 0.95 to be visible and minimal 0.1
            Dim L As Decimal = CDec(Math.Max(0.1, (Math.Min(Math.Sqrt(DeltaX * DeltaX + DeltaY * DeltaY), 0.95))))
    ...
    Private Function GetEnergy() As Decimal
        ...
        With Constants
            ...
            EKin = ThreadM * (v1 * v1 + 2 * v1 * v2 * CDec(Math.Cos(u2)) * .Component(0) + CDec(Math.Pow(v2 * .Component(0), 2))) / 2

            'potential energy: see math.doc.
            EPot = SpringM * v1 * v1 / 2 + MyD * u1 * u1 / 2 - ThreadM * g * CDec(Math.Cos(u2)) * .Component(0)
    ...
    Private Function G1(x As ClsVector) As Decimal
        ...
        With x
            ...
            Temp /= ThreadM + SpringM - CDec(Math.Pow(Math.Cos(.Component(2)), 2))
    ...
    Private Function G2(x As ClsVector) As Decimal
        ...
        With x
            ...
            Temp -= g * CDec(Math.Sin(.Component(2))) * (ThreadM + SpringM)
            Temp -= CDec(Math.Pow(.Component(3), 2) * MyConstants.Component(0) * Math.Sin(.Component(2)) * Math.Cos(.Component(2)))
            Temp /= MyConstants.Component(0) * (ThreadM + SpringM - CDec(Math.Pow(Math.Cos(.Component(2)), 2)))
