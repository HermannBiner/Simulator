'This class is the Real Thread Pendulum
'together with the "Runge Kutta 4" numerical Method 
'based on the differential equation of a real thread pendulum
'y"+sin(y)=0 -- y in [-1,1]

'Status Checked

Public Class ClsRealThreadPendulum
    ...
    Protected Overrides Sub Iteration()
        ...
        With MyActualParameter
            For i = 1 To MyNumberOfApproxSteps
                ...
                k.Component(0) = MyH * v
                l.Component(0) = -MyH * CDec(Math.Sin(u))

                k.Component(1) = MyH * (v + l.Component(0) / 2)
                l.Component(1) = MyH * (-CDec(Math.Sin(u + k.Component(0) / 2)))

                k.Component(2) = MyH * (v + l.Component(1) / 2)
                l.Component(2) = MyH * (-CDec(Math.Sin(u + k.Component(1) / 2)))

                k.Component(3) = MyH * (v + l.Component(2))
                l.Component(3) = MyH * (-CDec(Math.Sin(u + k.Component(2) / 2)))
