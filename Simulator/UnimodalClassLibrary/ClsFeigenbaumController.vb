...
Public Class ClsFeigenbaumController
    ...
    Private Sub DrawSplitPoints()
        ...
        For i = 0 To Splitpoints.Count - 1
            If DS.FormulaParameter.Range.IsNumberInInterval(Splitpoints(i)) Then
                A.X = Splitpoints(i)
                A.Y = DS.ValueParameter.Range.A
                B.X = Splitpoints(i)
                B.Y = DS.ValueParameter.Range.B
                If i < Splitpoints.Count - 1 Then
                    BmpGraphics.DrawLine(A, B, Color.Green, 1)
                Else
    ...
    Private Function IsUserDataOK() As Boolean
        ...
        Dim CheckParameterRange As New ClsCheckUserData(MyForm.TxtAMin, MyForm.TxtAMax, DS.FormulaParameter.Range)
